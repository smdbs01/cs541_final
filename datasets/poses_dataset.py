...
def read_pose_file(filepath):
    ...
    save_to = os.path.join(
        "/home/dxli/workspace/nslt/code/Pose-GCN/posegcn/features", vid
    )
    ...
    try:
        ...
        x = [
            v
            for i, v in enumerate(body_pose)
...
class Sign_Dataset(Dataset):
    def __init__(
        ...
        assert os.path.exists(index_file_path), (
            "Non-existent indexing file path: {}.".format(index_file_path)
        )
        assert os.path.exists(pose_root), "Path to poses does not exist: {}.".format(
            pose_root
        )

        self.data = []
        self.label_encoder, self.onehot_encoder = (
            LabelEncoder(),
    ...
    def __getitem__(self, index):
        ...
        x = self._load_poses(
            video_id, frame_start, frame_end, self.sample_strategy, self.num_samples
        )
        ...
        if self.test_index_file is not None:
            print(
                "Trained on {}, tested on {}".format(
                    index_file_path, self.test_index_file
                )
            )
    ...
    def _load_poses(
        self, video_id, frame_start, frame_end, sample_strategy, num_samples
    ):
        ...
        if sample_strategy == "rnd_start":
            ...
            raise NotImplementedError(
                "Unimplemented sample strategy found: {}.".format(sample_strategy)
            )
...
def k_copies_fixed_length_sequential_sampling(
    frame_start, frame_end, num_samples, num_copies
):
    num_frames = frame_end - frame_start + 1
    ...
    if num_frames <= num_samples:
        ...
        for i in range(num_copies):
            frames_to_sample.extend(
                list(
                    range(
                        frame_start + i * stride, frame_start + i * stride + num_samples
                    )
                )
            )
